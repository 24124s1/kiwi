...
menu.bg.pre.Text = 'kiwi<font color="#c375ae">.vip</font> 																				 <font color="#c375ae">(dev build)</font>'
...
function toggle:addKeybind(args)
    ...
    contextFrame.Parent = keybindFrame
    contextFrame.BackgroundColor3 = Color3.fromRGB(30, 30, 30)
    contextFrame.BorderSizePixel = 0
    contextFrame.Position = UDim2.new(0, button.Position.X.Offset, 1, 0)
    contextFrame.Size = UDim2.new(0, 70, 0, 0)
    contextFrame.Visible = false
    contextFrame.ZIndex = 10
    ...
    contextLayout.HorizontalAlignment = Enum.HorizontalAlignment.Left
    contextLayout.SortOrder = Enum.SortOrder.LayoutOrder
    contextLayout.Padding = UDim.new(0, 0)
    contextLayout.Parent = contextFrame

    local function buildDropdown()
        contextFrame:ClearAllChildren()
        contextLayout.Parent = contextFrame

        for _, m in ipairs(modes) do
            local opt = Instance.new("TextButton")
            opt.Parent = contextFrame
        ...
        contextFrame.Size = UDim2.new(0, 70, 0, #modes * 18)
    end
    ...
    inputService.InputBegan:Connect(function(key)
        ...
        elseif key == library.flags[args.flag] then
            if mode == "Always" or (mode == "Toggle" and not toggle._toggled) or mode == "Hold" then
                args.callback()
                toggle._toggled = not toggle._toggled
            end
        end
    end)
    ...
    inputService.InputEnded:Connect(function(key)
        ...
        if mode == "Hold" and k == library.flags[args.flag] then
    ...
    updateValue(args.key or Enum.KeyCode.Unknown)
        ...
        function group:addSlider(args)
            ...
            fill.Size = UDim2.new(0.617, 0, 1, 0)
            ...
            local function updateValue(value)
                if library.colorpicking then return end
                local fillSize = (value ~= 0) and UDim2.new(value / args.max, 0, 1, 0) or UDim2.new(0, 1, 1, 0)
                fill:TweenSize(fillSize, Enum.EasingDirection.In, Enum.EasingStyle.Sine, 0.01)
                valuetext.Text = value .. (args.suffix or "")
                library.flags[args.flag] = value
                if args.callback then
                    args.callback(value)
                end
...
function library:createConfig()
    ...
    if table.find(library.options["selected_config"].values, name .. ".cfg") then
            ...
            pcall(function()
                if config[i] ~= nil then
                    if v.type == "colorpicker" then
                        v.changeState(Color3.new(unpack(config[i])))
                    ...
                    -- Reset to default if missing in config
...
function library:refreshConfigs()
    ...
    for _, v in next, listfiles("OsirisCFGS") do
        local name = string.match(v, "OsirisCFGS[\\/](.+)%.cfg$")
        if name then
